...
public class VESIT_3040301_104_Assign5_Sairam {
    public static void main(String args[]) throws IOException, FileNotFoundException {
        String filename = "C:/Users/saira/Documents/Ganit-internship/Assignment Excel/VESIT_3040301_104_Assign5_Sairam.xlsx";     //Location where excel file is getting generated
        ...
        for (int k = 1; k < q + 1; k++) {
            ...
            while (coeff2>=0 || Math.abs(coeff2)>=coeff1){
            ...
            for (int j = 0; j < 5; j++) {
                wrongAnswer[j] = constructPolynomialTerm(coeffW[j], variable, alphaIndex);
                if (wrongAnswer[j].contains("0")){
                    wrongAnswer[j] = "None of the above<br># दिलेल्या पैकी कोणतेही नाही";
                }
            }
            String termAns = constructPolynomialTerm(coeff3, variable, alphaIndex);
            ...
            String term1 = constructPolynomialTerm(coeff1, variable, alphaIndex);
            String term2 = constructPolynomialTerm(coeff2, variable, alphaIndex);
            String correctAnswer = constructPolynomialTerm(coeffAnswer, variable, alphaIndex);
            String answerNeed = constructPolynomialTerm(1, variable, alphaIndex);
...
//			  System.out.println(rowTotal);
    ...
    static String constructPolynomialTerm(int coefficients, String[] variable, int[] alphaIndex) {
        ...
        for (int i = 0; i < 2; i++) {
            if(i==0) {
                ...
                if (Math.abs(coefficients) != 1) {
                    polynomialBuilder.append(Math.abs(coefficients));
                }
            }
            polynomialBuilder.append(variable[alphaIndex[i]]);
        }
